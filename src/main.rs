...
use tree_printer::{
    printers::{
        traditional::{aligners::DefaultAligner, liners::DefaultLiner, TraditionalTreePrinter},
        TreePrinter,
    },
...
fn main() -> Result<()> {
    ...
    TraditionalTreePrinter::<_, DefaultAligner, DefaultLiner>::default()
        .print(root_node, stdout())
...
fn build_tree() -> Box<dyn TreeNode> {
    ...
    let mut sub_node1 = Box::new(SimpleTreeNode::new("SUB asdf\nSSS fdsa\nxxx yyy"));
    ...
    // root_node.add_child(Box::new(PlaceholderTreeNode::new()));
...
//     root.add_child(Box::new(SimpleTreeNode::new("Child #2")));
//     root
// }
